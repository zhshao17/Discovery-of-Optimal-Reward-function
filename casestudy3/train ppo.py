...
def main(args):
    ...
    reward_funciton = reward_machine.RewardFunction(env=env, args=args, device=device)
    ...
    if (env.train):
        ...
        for epoch in range(1, args.number_epochs):
            ...
            while ((not game_over) and timestep <= 5 * 30 * 24 * 60):
                ...
                reward = reward_funciton.get_reward(np.array(current_state), action, np.array(next_state)) 
            ...
            epidata = reward_funciton.store_V(epidata)
            
            reward_funciton.D_xi.append(epidata)
            
            reward_funciton.optimaze_reward(agent=ppo)

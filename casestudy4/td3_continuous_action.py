...
"""
    ...
    # Reward Setup
    reward_funciton = RewardFunction(teacher_name = args.teacher_name, env=envs, args = args, device = device)
    ...
    for global_step in range(args.total_timesteps):
        ...
        with torch.no_grad():
            rewards = reward_funciton.observe_reward(obs, actions, next_obs)
        ...
        if "final_info" in infos:
            for info in infos["final_info"]:
                if info is not None:
                    ...
                    reward_funciton.D_xi.append(reward_funciton.store_V(epidata)) 
                    epidata = []
                    break
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % args.policy_frequency == 0:
                actor_loss = -qf1(data.observations, actor(data.observations)[0]).mean()
            ...
            if global_step % args.reward_frequency == 0:
                reward_funciton.optimaze_reward(agent=actor)
